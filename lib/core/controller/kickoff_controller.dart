...
class KickOffController {
  final sharedPreferencesController =
      DependencyInjection.getDependency<SharedPreferencesController>();

  Future<bool> _checkIfOnboardingIsShown() async {
    final onboardingShown = await sharedPreferencesController
            .getBool(SharedPreferencesKeys.onBoardingShown) ??
        false;
  ...
  Future<void> kickOff(BuildContext context) async {
    ...
    await sharedPreferencesController.setBool(
        SharedPreferencesKeys.onBoardingShown, true);
